...
## 정정 2 (2026-08-05, `06` 착수 전) — 맵 표기 전면 교체

**이 문서의 ASCII 표기 예시는 더 이상 유효하지 않다.** 경계 벽이 도입되면서 칸만 나열하는 표기로는 맵을 적을 수 없게 됐다.

| 이 문서의 것 | 실제 |
|---|---|
| `rows: ['......', '.@....', ...]` (N줄) | 칸과 경계를 번갈아 적는 **`2N+1` 격자** (기획서 §9.1) |
| 검증: 행 길이 · 기호 · 플레이어 1개 · 목표 1개 · 정지 요소 | + **격자 구조 검증** (홀수 크기, 교차점 `+`, 자리별 허용 기호, 외곽 닫힘) |
| 레벨 6개 | **7개** — 경계 벽을 도입하는 레벨 7 추가 |

**기존 6개 레벨의 판 내용은 그대로 옮겼고, `minMoves` 가 전부 그대로 통과한 것이 변환이 정확했다는 증거다.** BFS 솔버를 만들어둔 것이 여기서 값을 했다.

표기가 길어졌지만(6×6 → 13줄) 얻은 것이 있다. **격자 구조 자체가 검증 대상이라 열이 하나 밀리는 종류의 오타는 레벨을 실행해보기 전에 잡힌다.**
