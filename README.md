...
> 블록은 미끄러지는 연출로 움직이고, 되돌리기는 **레벨당 3회**다(다시하기로 되살아난다).
> 다만 **진행도가 저장되지 않고**, 레벨 선택 화면은 자리표시자다.
...
- **되돌리기는 레벨당 3회짜리 자원이다.** 무제한이면 아무 방향이나 눌러보고 무르는 것이
  최적 전략이 되어 퍼즐이 성립하지 않는다. 다시하기가 횟수를 되살리므로 막히지는 않는다.
...
**feature 의존은 `game → level` 한 방향뿐이다.** `level`은 판을 모른다 — 레벨 목록에 필요한 건
번호 · 이름 · 별점뿐이고, 맵을 `Level`에 품게 하면 두 feature가 서로를 참조하게 된다.
