...
> **현재 상태: 규칙 엔진과 레벨 데이터까지 완료, 화면 미구현.**
> **단위 테스트로 고정된 이동 규칙 엔진**과 6개 튜토리얼 레벨이 동작하고,
> 각 레벨의 최소 이동 횟수는 완전 탐색 솔버가 검증한다.
> 보드 렌더링과 입력이 아직 없어 두 화면은 자리표시자다.
