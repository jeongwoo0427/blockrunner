    ...
    return Center(
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // 격자가 정사각이 아니어도 셀은 정사각이어야 한다.
          final cell = extent / max(board.rowCount, board.colCount);

          return SizedBox(
            width: cell * board.colCount,
            height: cell * board.rowCount,
            child: Stack(
              children: [
                Positioned.fill(
                  child: CustomPaint(
                ...
                for (final block in board.blocks)
                  Positioned(
                    ...
                    left: block.position.col * cell,
                    top: block.position.row * cell,
                    width: cell,
                    height: cell,
