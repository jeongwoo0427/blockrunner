...
void main() {
  ...
  group('파서는 풀이 가능성을 판단하지 않는다 (기획서 §9.2)', () {
    test('사방에 정지 요소가 없는 목표도 그대로 읽는다', () {
      // 목표(2,2)는 맵 경계에도 닿지 않고 벽도 인접하지 않는다. 한때 파서가 이런
      // 판을 "멈출 수 없다" 며 거절했지만 **의도한 배치를 막았다** — 동료를 흘려
      // 넣어 그 앞에 서면 되고(기획서 §4.4-1 발판), 인접 칸만 보는 검사로는 그것을
      // 알 수 없다. 실제로 풀리는지는 `level_design_test` 의 완전 탐색이 본다.
      expect(
        () => parser.parse(
          blueprintOf(const [
            '+-+-+-+-+-+',
    ...
    test('목표를 둘러싼 배치가 무엇이든 읽는다', () {
      // 맵 경계 · 칸 벽 · 경계 벽 — 어느 것이 목표에 붙어 있든 파서에게는 같다.
      // 예전에는 이 셋만이 판을 통과시키는 이유였다.
      for (final row in ['|. @ . . G|', '|. @ G # .|', '|. @ G|. .|']) {
        expect(
          () => parser.parse(
            blueprintOf([
              '+-+-+-+-+-+',
